			...
			<nav>
				<ul className='flex gap-4 lg:flex-col text-sm'>
					...
					<li className={`relative transition-all ${disabled('skip') ? 'grayscale cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}>
						<button autoFocus onClick={useSkipCard} className='p-[10px] aspect-square rounded bg-blue-500 w-full' title='Асуултыг алгасах' disabled={disabled('skip')}>
					...
					<li className={`relative transition-all ${disabled('half') ? 'grayscale cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}>
						<button onClick={useHalfCard} className='p-[10px] aspect-square rounded bg-blue-500 w-full ' title='50:50 боломж' disabled={disabled('half')}>
					...
					<li className={`p-[10px] relative flex items-center rounded bg-blue-500 ${wildCards.lives < 1 ? 'grayscale' : ''}`} title='Амь'>

...
export default function JsxForm ({ handleInputs, nowQueries }) {
	...
	const WILCARDS = [
		{ name: 'Асуултыг алгасах', icon: <BsSkipEndFill color='white' className='text-2xl' />, amount: 1 },
		{ name: '50:50 боломж', icon: <Image src={fiftyImg.src} alt="fifty fifty" width={23} height={23} />, amount: 1 },
		{ name: 'Амь', icon: <FaHeart color='white' className='text-2xl' />, amount: 1 }
	]
		...
		<>
			<div className='flex gap-2 sm:gap-5 flex-col'>
				...
				<fieldset className='p-1 relative'>
					<legend className='text-lg font-semibold mb-2'>Асуултууд</legend>
				...
				<fieldset className='p-1 relative'>
					<legend className='text-lg font-semibold mb-2'>Хугацаа</legend>
			...
			<fieldset>
				<legend className='text-lg font-semibold mb-2 mx-1'>Бүлгүүд</legend>
					...
					{categoriesJSON.map(category => (
						<label key={category.id} className="relative cursor-pointer" title={category.name}>
							...
							<Image className={`absolute transition-all w-full h-full peer-checked:scale-90 p-2 rounded peer-checked:bg-[${category.color}] 
												invert peer-checked:invert-0 peer-checked:bg-[var(--bgColor)] top-0 pointer-events-none peer-checked:outline-2 
												peer-checked:outline-offset-2 peer-checked:outline outline-[var(--bgColor)]`} src={`/categories-icons/${category.name.toLowerCase()}.svg`} 
												alt={category.name} width={40} height={40} style={{ '--bgColor': category.color }} />
						</label>
					))}
				</div>
			</fieldset>
